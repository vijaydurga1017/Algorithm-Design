...
def greedy_TSP(Adj_Matrix,dict_adj_list,start_node,next_node):
	...
	for i in range(0,len(Adj_Matrix)):
		...
		for j in range (0,len(Adj_Matrix)):
			...
			if minimum_value > value:
				minimum_value = value
				start_node = next_node = i+1
			
	final_path = []
	path_dist = greedy_TSP(Adj_Matrix,dict_adj_list,start_node,next_node)
	path = path_dist[0]
	last_node = path[-1]
	cost = path_dist[1]
